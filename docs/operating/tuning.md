...
| `config/settings.yaml` · `models.analyze` = `claude-sonnet-5` | back to `claude-opus-5` ($5/$25 per MTok vs $3/$15) buys judgment on the one call that *is* the product | a cheaper model here is the one substitution that changes *what the tool is*: the ranking is the product | rankings you stop trusting — a role you would apply to sitting below one you would not. **The specific tripwire: the calibration cases in `profile/rubric.md` starting to mis-score again** — a Coordinator role scoring 80+, or a mandatory-tech gap surviving into the apply set. Moved off Opus 2026-07-29 for cost (see `llm.effort` below); this is the first line to revert |
| `config/settings.yaml` · `llm.effort` = `medium` | `high`/`xhigh`/`max` think longer per job — and since billing here is **output-token dominated**, effort is the dominant cost lever, not `max_workers` or the model id alone | `low` on a rubric this subtle risks scoring on keywords rather than on role shape | a bill that moves without the job count moving. Measured 2026-07-29: 2,050 jobs scored in July on Opus 5 at effort `high` ≈ **$100**, essentially all of it output — the input side is already cached (`triage/analyze.py` marks the rubric `cache_control`, so its ~2,400 tokens bill at 10% rather than 2,050 times). Unset = the provider's default (`high` on Anthropic); dropped with a warning on a provider that does not declare support |
...
| `cv/review_cv.py` · `MAX_PASSES` = 2 | tokens, and they are the single biggest line in a run — CV agents were 55% of a 6.15M-token day | the grader gets another go at a document that genuinely needed it | a document left one placement fix short. **Cut from 3 on 2026-08-04 on measurement:** across 19 documents the third pass changed nothing on 8 of 10, because the residue was `absent` terms with no evidence — and it was where the grader pushed hardest for the fixes that must be refused (SQL, CI/CD, invented metrics), so it spent tokens generating pressure the agent then had to resist |
| `cv/review_cv.py` · `PASS_BAR` = 4 | a document must clear 4 on **every** dimension, not on a mean | a mean would let a strong keyword match paper over a CV that sells nothing | most documents stop below bar on `pitch_fit`, and that is usually a true fact about the requisition rather than a defect. **Not a tuning knob:** lowering it makes "passes" mean nothing |
